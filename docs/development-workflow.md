...
npm run nswag:generate  # NSwag コード生成のみ (コントローラー + API クライアント)
...
```
admin.backend/Presentation/nswag.json                ← admin バックエンド用 (コントローラー + DTO)
admin.backend/Infrastructure/nswag.gameserver-client.json ← game.server API クライアント生成用
game.server/Presentation/nswag.json                  ← ゲームサーバー用 (コントローラー + DTO)
```
...
各アプリケーションのテーブルは、開発環境では EF Core の `Database.Migrate()` により起動時にマイグレーションが自動適用されます。
